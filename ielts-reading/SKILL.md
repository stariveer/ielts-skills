...
### Phase 6：自动持久化与词库沉淀（落盘协议）

每次分析完成后，**自动执行以下数据持久化操作**：

1. **同义替换沉淀至 `data/paraphrases.md`**
   - 将本次 Phase 4 提炼的「同义替换词对」自动合并追加至 `data/paraphrases.md` 的「剑桥真题阅读同义替换库」表格中，标明出处（如 C14T2R1 或文章标题）。
   - 避免重复项，定期扩充用户个人词库。

2. **错因与陷阱归档至 `data/mistakes.md`**
   - 若出现典型混淆（如 False 与 Not Given 混淆、Heading 细节干扰等），将错题核心逻辑总结追加到 `data/mistakes.md`。

3. **联动更新真题进度 `data/mock-score.md`**
   - 若本次训练基于剑桥真题（如 C15T2R1），主动检查 `data/mock-score.md`：
     - 若该条目尚未填入得分，自动填入本次得分（如 `11/13`）、测试日期和做题耗时；
     - 若为二次精读，更新为如 `12/13(二)`。

4. **流水记录写入 `data/progress.md`**
   - 在 `data/progress.md` 的训练流水表中追加本次阅读精读记录，并更新同义替换累计词数。
